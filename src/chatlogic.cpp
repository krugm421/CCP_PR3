...
ChatLogic::~ChatLogic()
{
    ...
    // delete all nodes
    // Task 3: Deleting of GraphNode objects is no longer required as the handels are now store with unique_ptr
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            // Task 3. Allocating edge ^^^, moving ownership to parent node.
